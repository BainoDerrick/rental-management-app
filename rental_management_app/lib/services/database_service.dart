...
class DatabaseService {
  ...
  FirebaseFirestore get firestore =>
      _firestore; // Expose for use in HistoryScreen
  ...
  Stream<(List<Tenant>, DocumentSnapshot?)> getTenants({
    int limit = 20,
  ...
  Future<double> getRentBalance(String tenantId, String monthYear) async {
    ...
    final monthlyRent =
        (tenantDoc.data() as Map<String, dynamic>)['rentThreshold'] ?? 0.0;

    final paymentsResult = await getPaymentsForMonth(tenantId, monthYear).first;
    final totalPaid = paymentsResult.$1.fold(
      0.0,
      (sum, payment) => sum + payment.rentPaid,
    );

    return monthlyRent - totalPaid;
  }

  Stream<(List<Payment>, DocumentSnapshot?)> getPayments(
    String tenantId, {
    int limit = 20,
    DocumentSnapshot? startAfter,
  }) {
...
Future<List<Payment>> getPaymentsByTenant(String tenantId) async {
  try {
    ...
    if (kDebugMode) {
      print('Error fetching payments for tenant $tenantId: $e');
    }
